                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-xs font-bold text-brand-text-secondary uppercase mb-1">
                                {isCreating ? 'Password *' : 'Reset Password'}
                            </label>
                            <input
                                type="password"
                                required={isCreating}
                                ...
                                placeholder={isCreating ? "Crea password" : "Lascia vuoto per mantenere"}
                            />
                        </div>
                        <div>
                            <label className="block text-xs font-bold text-brand-text-secondary uppercase mb-1">Crediti</label>
                            <input required type="number" className="w-full bg-black/40 border border-white/10 focus:border-brand-accent p-3 rounded text-white outline-none transition-colors" value={credits} onChange={e => setCredits(parseInt(e.target.value))} />
                        </div>
                    </div>
...
export const AdminPanel: React.FC = () => {
    ...
    useEffect(() => {
        ...
        let interval: any;
        if (activeTab === 'overview') {
            interval = setInterval(loadData, 15000); // Reduced poll frequency
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const finalData = {
            ...
            videoUrl: mediaType === 'image' ? '' : formData.videoUrl
        };
                ...
                <div className="animate-fade-in space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        ...
                        <StatCard
                            ...
                            value={`$${stats.apiStatus?.paddle?.used || 0}`} // FIX: Safe access
                        ...
                        <StatCard
                            title="Server Status"
                            value="ONLINE"
                            subtext={`CPU: ${stats.serverHealth?.cpu || 0}% | RAM: ${stats.serverHealth?.memory || 0}%`} // FIX: Safe access
                            icon="fa-server"
                            ...
                            {stats.apiStatus && (
                                <div className="space-y-6">
                                    <ProgressBar
                                        ...
                                        value={Math.round(stats.apiStatus.gemini?.used || 0)}
                                        max={stats.apiStatus.gemini?.limit || 10000}
                                        unit="tok"
                                        colorClass="bg-blue-500"
                                    />
                                    <p className="text-xs text-right text-brand-text-secondary -mt-3 mb-4">
                                        Costo Stimato: <span className="text-white font-bold">${(stats.apiStatus.gemini?.costEstimated || 0).toFixed(3)}</span>
                                    </p>

                                    <ProgressBar
                                        label="Cloud Storage (S3 Bucket)"
                                        value={stats.apiStatus.storage?.used || 0}
                                        max={stats.apiStatus.storage?.limit || 100}
                                        unit="GB"
                                        colorClass="bg-orange-500"
                                    />
                                    <p className="text-xs text-right text-brand-text-secondary -mt-3 mb-4">
                                        Costo Stimato: <span className="text-white font-bold">${(stats.apiStatus.storage?.costEstimated || 0).toFixed(2)}</span>
                                    </p>
                                </div>
                            )}
                        </div>
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex items-center justify-between p-3 bg-yellow-900/20 rounded border border-yellow-800/50">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className="text-white">Audio Generation Engine</span>
                                    </div>
                                    <span className="text-xs font-bold text-yellow-400 px-2 py-1 bg-yellow-900/40 rounded">CLIENT-SIDE</span>
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            <tbody className="text-sm divide-y divide-white/5">
                                {logs.map(log => (
                                    <tr key={log.id} className="hover:bg-white/5 transition-colors font-mono text-xs">
                                        ...
                                        <td className="p-4">
                                            <span className={`px-2 py-1 rounded uppercase font-bold ${log.level === 'error' ? 'bg-red-900/50 text-red-300 border border-red-900' :
