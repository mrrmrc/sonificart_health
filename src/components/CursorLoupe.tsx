...
export const CursorLoupe: React.FC<CursorLoupeProps> = React.memo(({ activeEvent, isPlaying }) => {
    ...
    useEffect(() => {
        ...
        if (activeEvent?.sourceBlock) {
            ...
            // Draw the solid average color of the block for perfect consistency
            ctx.fillStyle = `rgb(${Math.round(r)}, ${Math.round(g)}, ${Math.round(b)})`;
            ctx.fillRect(0, 0, canvas.width, canvas.height);
        } else {
            // Clear canvas if no event is active (e.g., at the end of playback)
            ...
            {(isPlaying || activeEvent) && (
                 <div className="flex gap-3 items-center animate-fade-in pt-2">
                    ...
                    <div className="flex-grow space-y-1">
                        ...
                        <DataRow label="Coordinate" value={`[${activeEvent?.sourceBlock.position.x}, ${activeEvent?.sourceBlock.position.y}]`} />
